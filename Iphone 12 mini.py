...
iurl <- "https://www.imdb.com/title/tt9140560/reviews?ref_=tt_ov_rt"
imdb_reviews <- NULL
for (i in 1:10){
...
length(imdb_reviews)
write.table(imdb_reviews,"WandaVision.txt",row.names = F)
getwd()
wandavision <- read.delim('WandaVision.txt')
str(wandavision)
View(wandavision)
...
library(dplyr)
# Read File 
imdb_reviews <- read.delim('WandaVision.TXT')
...
# barplot 
barplot(colSums(s), las = 2.5, col = rainbow(10),ylab = 'Count',main= 'Sentiment scores for IMDB Reviews for WandaVision')
